...
public struct FileResolver {
    ...
    public static let isRanInsideXcode: Bool = { () -> Bool in
        let isUsedInXcodeLibrary = executableURL.path.range(of: "/DerivedData/") != nil
        let isUsedInXcodeXCTest = URL(fileURLWithPath: "").path == "/private/tmp"
        return isUsedInXcodeLibrary || isUsedInXcodeXCTest
    }()
    ...
    public static let presentWorkingDirectory: String = presentWorkingDirectoryURL.path
    ...
    public static let presentWorkingDirectoryURL: URL = { () -> URL in
    ...
    public static let executableFolderURL: URL = { () -> URL in
        if isRanInsideXcode || isRanFromXCTest {
            ...
            if let range = sourceFile.range(of: "/checkouts/") {
                // In Swift 3.1 and newer, package source code is downloaded to /<build-path>/checkouts
